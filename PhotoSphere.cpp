...
PhotoSphere *PhotoSphere::FromFile(const string &filename, uint32_t hslice, uint32_t vslice)
{
    ...
    NULLPTREXAM(ret != nullptr, "Creating class PhotoSphere failed!", NULLFUNCTION);
    PhotoSphereProperty *property = PhotoSphereProperty::FromFile(filename);
    NULLPTREXAM(property != nullptr, "Creating class PhotoSphereProperty failed!", ret->Dispose);
    ret->m_property = property;
    ret->m_shader = new Shader(m_VertexShaderString.data(), m_FragmentShaderString.data());
    NULLPTREXAM(ret->m_shader != nullptr, "Creating shader failed!", ret->Dispose);
    ...
    NULLPTREXAM(ret->m_vao != nullptr, "Creating Vertex Array failed!", ret->Dispose);
    ret->m_vbo = Buffer::CreateBuffer<BufferObject>();
    NULLPTREXAM(ret->m_vbo != nullptr, "Creating Vertex Buffer failed!", ret->Dispose);
    ret->m_vbo->SetFormat(vertices);
    ret->m_tbo = Buffer::CreateBuffer<BufferObject>();
    NULLPTREXAM(ret->m_tbo != nullptr, "Creating UV Buffer failed!", ret->Dispose);
    ret->m_tbo->SetFormat(texCoords);
    ret->m_ibo = Buffer::CreateBuffer<BufferObject>();
    NULLPTREXAM(ret->m_ibo != nullptr, "Creating Index Buffer failed!", ret->Dispose);
    ret->m_ibo->SetFormat(indices);
    Image *image = Image::ReadImage(filename);
    NULLPTREXAM(image != nullptr, "Cannot read " + filename +  "!", ret->Dispose);
...
{
#define DELETEPTR(p) if ((p) != nullptr) delete (p);
    DELETEPTR(m_property);
    DELETEPTR(m_shader);
    DELETEPTR(m_texture);
    DELETEPTR(m_vao);
    DELETEPTR(m_vbo);
